...
// --- DÉBUT MODIFICATION : Calculs dynamiques pour les stats ---
$active_sensors = 0;
if (!empty($data['temp_sensor'])) $active_sensors++;
if (!empty($data['gas_sensor'])) $active_sensors++;
if (!empty($data['light_sensor'])) $active_sensors++;
if (!empty($data['sound_sensor'])) $active_sensors++;
if (!empty($data['parking_sensors'])) $active_sensors += count($data['parking_sensors']);
// --- FIN MODIFICATION ---
?>
...
<div class="dashboard-container">
    ...
    <div class="sensors-stats">
        <div class="stat-card active-sensors">
            ...
            <div class="stat-info">
                <!-- MODIFICATION : Stat dynamique -->
                <div class="stat-number"><?= $active_sensors ?></div>
                <div class="stat-label">Capteurs Actifs</div>
            </div>
        </div>
        <div class="stat-card inactive-sensors">
            <div class="stat-icon"><i class="fas fa-exclamation-triangle"></i></div>
            <div class="stat-info"><div class="stat-number">0</div><div class="stat-label">En Maintenance</div></div>
        </div>
        <div class="stat-card temperature">
            <div class="stat-icon"><i class="fas fa-thermometer-half"></i></div>
            <div class="stat-info">
                <!-- MODIFICATION : Stat dynamique -->
                <div class="stat-number"><?= !empty($data['temp_sensor']) ? number_format($data['temp_sensor']['valeur'], 1) . '°C' : 'N/A' ?></div>
                <div class="stat-label">Température</div>
            </div>
        </div>
        <div class="stat-card data-rate">
            <div class="stat-icon"><i class="fas fa-tachometer-alt"></i></div>
            <div class="stat-info"><div class="stat-number">2s</div><div class="stat-label">Fréquence</div></div>
        </div>
    </div>
    ...
    <div class="sensors-grid">
        ...
        <!-- --- DÉBUT MODIFICATION : Carte dynamique pour la température --- -->
        <?php if (!empty($data['temp_sensor'])): $sensor = $data['temp_sensor']; ?>
        <div class="sensor-card temperature-sensor">
            <div class="sensor-header">
                <div class="sensor-title"><i class="fas fa-thermometer-half"></i><h3>Capteur Température</h3></div>
            ...
            <div class="sensor-body">
                ...
                <div class="sensor-info-grid">
                    <div class="info-item"><span class="label">ID Capteur:</span><span class="value">TEMP_<?= $sensor['id'] ?></span></div>
                    <div class="info-item"><span class="label">Localisation:</span><span class="value">Zone A</span></div>
                    <div class="info-item"><span class="label">Dernière lecture:</span><span class="value"><?= date('H:i:s', strtotime($sensor['heure'])) ?></span></div>
                    <div class="info-item"><span class="label">Batterie:</span><span class="value">95%</span></div>
                </div>
                <div class="sensor-actions">
                    <button class="btn-action" onclick="calibrateSensor('TEMP_<?= $sensor['id'] ?>')"><i class="fas fa-cog"></i> Calibrer</button>
                    <button class="btn-action" onclick="viewHistory('TEMP_<?= $sensor['id'] ?>')"><i class="fas fa-chart-line"></i> Historique</button>
                </div>
            </div>
        </div>
        <?php endif; ?>

        <!-- --- DÉBUT MODIFICATION : Carte dynamique pour le gaz --- -->
        <?php if (!empty($data['gas_sensor'])): $sensor = $data['gas_sensor']; ?>
        <div class="sensor-card gas-sensor">
            <div class="sensor-header">
                <div class="sensor-title"><i class="fas fa-smog"></i><h3>Capteur de Gaz</h3></div>
                <div class="sensor-status status-active"><i class="fas fa-circle"></i> Actif</div>
            </div>
            <div class="sensor-body">
                ...
                <div class="sensor-info-grid">
                    <div class="info-item"><span class="label">ID Capteur:</span><span class="value">GAS_<?= $sensor['id'] ?></span></div>
                    <div class="info-item"><span class="label">Type:</span><span class="value">MQ-135</span></div>
                    <div class="info-item"><span class="label">Dernière lecture:</span><span class="value"><?= date('H:i:s', strtotime($sensor['heure'])) ?></span></div>
                    <div class="info-item"><span class="label">Seuil:</span><span class="value">1000 ppm</span></div>
                </div>
                 <div class="sensor-actions">
                    <button class="btn-action" onclick="calibrateGas('GAS_<?= $sensor['id'] ?>')"><i class="fas fa-wind"></i> Calibrer</button>
                    <button class="btn-action" onclick="setGasAlert('GAS_<?= $sensor['id'] ?>')"><i class="fas fa-exclamation-triangle"></i> Alerte</button>
                </div>
            </div>
        </div>
        <?php endif; ?>

        <!-- --- DÉBUT MODIFICATION : Carte dynamique pour la lumière --- -->
        <?php if (!empty($data['light_sensor'])): $sensor = $data['light_sensor']; ?>
        <div class="sensor-card light-sensor">
            <div class="sensor-header">
                <div class="sensor-title"><i class="fas fa-sun"></i><h3>Capteur de Lumière</h3></div>
                <div class="sensor-status status-active"><i class="fas fa-circle"></i> Actif</div>
            </div>
            <div class="sensor-body">
                ...
                <div class="sensor-info-grid">
                    <div class="info-item"><span class="label">ID Capteur:</span><span class="value">LUM_<?= $sensor['id'] ?></span></div>
                    <div class="info-item"><span class="label">Type:</span><span class="value">Photorésistance</span></div>
                    <div class="info-item"><span class="label">Dernière lecture:</span><span class="value"><?= date('H:i:s', strtotime($sensor['heure'])) ?></span></div>
                    <div class="info-item"><span class="label">Mode:</span><span class="value">Auto</span></div>
                </div>
                <div class="sensor-actions">
                    <button class="btn-action" onclick="testLight('LUM_<?= $sensor['id'] ?>')"><i class="fas fa-lightbulb"></i> Test</button>
                    <button class="btn-action" onclick="setLightThreshold('LUM_<?= $sensor['id'] ?>')"><i class="fas fa-adjust"></i> Seuil</button>
                </div>
            </div>
        </div>
        <?php endif; ?>

        <!-- --- DÉBUT MODIFICATION : Carte dynamique pour le son --- -->
        <?php if (!empty($data['sound_sensor'])): $sensor = $data['sound_sensor']; ?>
        <div class="sensor-card sound-sensor">
            <div class="sensor-header">
                <div class="sensor-title"><i class="fas fa-microphone"></i><h3>Capteur Son</h3></div>
                <div class="sensor-status status-active"><i class="fas fa-circle"></i> Actif</div>
            </div>
            <div class="sensor-body">
                ...
                <div class="sensor-info-grid">
                    <div class="info-item"><span class="label">ID Capteur:</span><span class="value">SON_<?= $sensor['id'] ?></span></div>
                    <div class="info-item"><span class="label">Type:</span><span class="value">Electret</span></div>
                    <div class="info-item"><span class="label">Dernière lecture:</span><span class="value"><?= date('H:i:s', strtotime($sensor['heure'])) ?></span></div>
                    <div class="info-item"><span class="label">Seuil:</span><span class="value">85 dB</span></div>
                </div>
                <div class="sensor-actions">
                    <button class="btn-action" onclick="reconnectSound('SON_<?= $sensor['id'] ?>')"><i class="fas fa-plug"></i> Reconnecter</button>
                    <button class="btn-action" onclick="scheduleRepair('SON_<?= $sensor['id'] ?>')"><i class="fas fa-calendar"></i> Planifier</button>
                </div>
            </div>
        </div>
        <?php endif; ?>
        
        <!-- --- DÉBUT MODIFICATION : Boucle pour les capteurs de proximité --- -->
        <?php if (!empty($data['parking_sensors'])): ?>
            <?php foreach ($data['parking_sensors'] as $sensor): ?>
            <div class="sensor-card proximity-sensor">
                <div class="sensor-header">
                    <div class="sensor-title"><i class="fas fa-radar"></i><h3>Détecteur Place #<?= htmlspecialchars($sensor['place']) ?></h3></div>
                    <div class="sensor-status status-active"><i class="fas fa-circle"></i> Actif</div>
                </div>
                <div class="sensor-body">
                    ...
                    <div class="sensor-info-grid">
                        <div class="info-item"><span class="label">ID Capteur:</span><span class="value">PROX_<?= $sensor['id'] ?></span></div>
                        <div class="info-item"><span class="label">Type:</span><span class="value">Ultrasonique</span></div>
                        <div class="info-item"><span class="label">Dernière lecture:</span><span class="value"><?= date('H:i:s', strtotime($sensor['heure'])) ?></span></div>
                        <div class="info-item"><span class="label">Précision:</span><span class="value">±1 cm</span></div>
                    </div>
                    <div class="sensor-actions">
                        <button class="btn-action" onclick="testRange('PROX_<?= $sensor['id'] ?>')"><i class="fas fa-ruler"></i> Test portée</button>
                        <button class="btn-action" onclick="setThreshold('PROX_<?= $sensor['id'] ?>')"><i class="fas fa-sliders-h"></i> Seuil</button>
                    </div>
                </div>
            </div>
            <?php endforeach; ?>
