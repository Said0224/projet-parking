...
print(f"Gestion de la place '{PARKING_SPOT_NUMBER}' et de la LED ID '{LED_ID_IN_DB}'.")
...
def send_update_to_server(spot_number, status):
    """Envoie une mise à jour de statut au serveur (POST) ET réinitialise la LED en mode auto."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            set_led_to_auto_mode(status) # Le capteur a toujours la priorité
    ...
    while True:
        ...
        if line:
            ...
            if line.startswith("SENSOR_STATUS:"):
                send_update_to_server(line.split(':')[1], line.split(':')[2])
